...
int ProgAlgSPIFlash::spi_flashinfo_is25(unsigned char * buf)
{
  fprintf(stderr, "Found ISSI Device, Device ID 0x%02x%02x\n",
	  buf[1], buf[2]);
  if ((buf[1] != 0x60) && (buf[1] != 0x70))
    {
      fprintf(stderr,"IS25: Unexpected RDID upper Device ID 0x%02x\n", buf[1]);
      return -1;
    }
  switch (buf[2])
    {
    ...
    case 0x19:	// 256MBit
      ...
      break;
    default:
      fprintf(stderr,"Unexpected IS25 size ID 0x%02x\n", buf[2]);
      return -1;
    }
  pgsize = 256;
  ...
  spi_xfer_user1(NULL,0,0,buf,8, 4);
  spi_xfer_user1(buf,6,5,NULL,0, 0);
